...
from peewee_manager import Room, Teacher, SchoolClass, Course, TeachingClass, _database
...
def upload_data():
    ...
    try:
        ...
        if errors:
            return jsonify({'count': imported_count, 'errors': '; '.join(errors[:5])}), 207
        return jsonify({'count': imported_count, 'errors': None})
    ...
    for i, row in enumerate(data_rows, start=2):
        try:
            ...
            if existing_room:
                ...
                Room.create(room_number=room_number, name=name, capacity=capacity, room_type=room_type)
                imported += 1
        except Exception as e:
    ...
    for i, row in enumerate(data_rows, start=2):
        try:
            ...
            if existing_teacher:
                ...
                Teacher.create(teacher_number=teacher_number, name=name, teachable_courses=teachable_courses_json, max_weekly_sessions=max_weekly)
                imported += 1
        except Exception as e:
            errors.append(f'第{i}行：{str(e)}')
    return imported, updated, skipped, errors
    ...
    for i, row in enumerate(data_rows, start=2):
        try:
            ...
            if existing_class:
                ...
                SchoolClass.create(class_number=class_number, name=name, student_count=student_count, department=department)
                imported += 1
        except Exception as e:
            errors.append(f'第{i}行：{str(e)}')
    return imported, updated, skipped, errors
    ...
    for i, row in enumerate(data_rows, start=2):
        try:
            ...
            if existing_course:
                ...
                course = Course.create(course_number=course_number, name=name, course_type=course_type, total_hours=total_hours, teacher=teacher, school_class=school_class)
                imported += 1
            existing_tc = TeachingClass.get_or_none((TeachingClass.course == course) & (TeachingClass.school_class == school_class) & (TeachingClass.teacher == teacher))
            if not existing_tc:
                TeachingClass.create(course=course, school_class=school_class, teacher=teacher)
        except Exception as e:
            errors.append(f'第{i}行：{str(e)}')
    return imported, updated, skipped, errors

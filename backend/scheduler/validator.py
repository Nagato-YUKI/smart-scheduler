...
class ScheduleValidator:
    ...
    def validate_schedule(self, schedule: Dict, courses: List, time_pool,
                          room_allocator, holiday_manager) -> ValidationResult:
            ...
            for session in sessions:
                ...
                if teacher_id:
                    if slot_key in teacher_slots[teacher_id]:
                        result.add_error(
                            f"教师 {teacher_id} 在时段 {slot_key} 有课程冲突"
                        )
                    teacher_slots[teacher_id].add(slot_key)

    def _check_room_conflicts(self, schedule: Dict, room_allocator,
                              result: ValidationResult):
            ...
            for session in sessions:
                ...
                if room_id:
                    if slot_key in room_slots[room_id]:
                        result.add_error(
                            f"教室 {room_id} 在时段 {slot_key} 有课程冲突"
                        )
            ...
            for session in sessions:
                ...
                if class_id:
                    if slot_key in class_slots[class_id]:
                        result.add_error(
                            f"班级 {class_id} 在时段 {slot_key} 有课程冲突"
                        )
                    class_slots[class_id].add(slot_key)

    def _check_holiday_conflicts(self, schedule: Dict, time_pool,
                                 holiday_manager, result: ValidationResult):
        for slot_key, sessions in schedule.items():
            ...
            if len(parts) >= 3:
                ...
                if date and holiday_manager.is_holiday(date):
                    result.add_error(
                        f"时段 {slot_key} 安排在节假日 {date.strftime('%Y-%m-%d')}"
                    )
        ...
        for teacher_id, week_counts in teacher_week_sessions.items():
            for week, count in week_counts.items():
                if count > self.max_teacher_sessions_per_week:
                    result.add_error(
                        f"教师 {teacher_id} 在周次 {week} 安排了 {count} 课时，超过上限 {self.max_teacher_sessions_per_week}"
                    )
        ...
        for class_id, day_counts in class_day_sessions.items():
            for day_key, count in day_counts.items():
                if count > self.max_class_sessions_per_day:
                    result.add_error(
                        f"班级 {class_id} 在 {day_key} 安排了 {count} 课时，超过上限 {self.max_class_sessions_per_day}"
                    )

    def _check_all_courses_scheduled(self, schedule: Dict, courses: List,
                                     result: ValidationResult):
    ...
    def _check_fixed_room_constraints(self, schedule: Dict, courses: List,
                                      result: ValidationResult):
            ...
            for session in sessions:
                ...
                if course_id and course_id in course_map:
                    ...
                    if fixed_room and room_id != fixed_room:
                        result.add_error(
                            f"课程 {course_id} 应使用固定教室 {fixed_room}，但实际使用了 {room_id}"
                        )

    def validate_single_session(self, session: Dict, schedule: Dict,
                                room_allocator, constraint_checker,
                                time_pool, holiday_manager) -> ValidationResult:
        result = ValidationResult()
        ...
        if not all([teacher_id, class_id, room_id, week, day, period]):
            result.add_error("会话信息不完整")
            return result
        ...
        if not time_pool.is_available(week, day, period):
            date = time_pool.get_date(week, day)
            if date and holiday_manager.is_holiday(date):
                result.add_error(f"时段 {slot_key} 是节假日")
            else:
                result.add_error(f"时段 {slot_key} 不可用")

        for key, sessions in schedule.items():
            for s in sessions:
                if key == slot_key:
                    if s.get('teacher_id') == teacher_id:
                        result.add_error(f"教师 {teacher_id} 在 {slot_key} 有冲突")
                    if s.get('class_id') == class_id:
                        result.add_error(f"班级 {class_id} 在 {slot_key} 有冲突")

        if not room_allocator.is_room_available(room_id, week, day, period):
            result.add_error(f"教室 {room_id} 在 {slot_key} 有冲突")

<template>
  <div class="schedule-page">
    <div class="schedule-toolbar">
      <h2 class="page-title"><span class="title-icon">📅</span>教师课表</h2>
    ...
    <div v-if="!selectedTeacherId" class="empty-tip"><el-empty description="请选择教师查看课表" /></div>
    ...
    <div v-else>
      <div class="info-bar teacher-info" v-if="selectedTeacher">
        <span class="info-item"><span class="info-label">教师：</span><span class="info-value">{{ selectedTeacher.name }}</span></span>
        <span class="info-sep">|</span>
        <span class="info-item"><span class="info-label">工号：</span><span class="info-value">{{ selectedTeacher.teacher_number }}</span></span>
        <span class="info-sep">|</span>
              ...
              <tr v-for="(period, pIdx) in section.periods" :key="pIdx" :class="{ 'section-break': pIdx === section.periods.length - 1 }">
                ...
                <td class="schedule-cell" v-for="day in weekDays" :key="day.index">
                  <div v-if="getCellContent(day.index, period)" class="course-card" :style="getCardStyle(getCellContent(day.index, period))">
                    ...
                    <div class="course-detail"><span class="icon">👥</span><span>{{ getCellContent(day.index, period).class_name }}</span></div>
...
function handlePrint() { printSchedule('教师课表') }

async function loadTeacherList() {
  try {
    const res = await teacherApi.getList({ page: 1, per_page: 100 })
    teacherList.value = res.teachers || []
    if (teacherList.value.length > 0 && !selectedTeacherId.value) { selectedTeacherId.value = teacherList.value[0].id; loadSchedule() }
  } catch (e) { console.error(e) }
}

async function loadSchedule() {
  if (!selectedTeacherId.value) { courseData.value = []; return }
  loading.value = true
  try {
    ...
    if (!courseData.value.length) ElMessage.warning('该教师本周暂无排课数据')
  } catch (e) { console.error(e); ElMessage.error('加载课表失败') } finally { loading.value = false }
}

onMounted(() => { loadTeacherList() })
</script>

<style scoped>.teacher-info { background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); }</style>

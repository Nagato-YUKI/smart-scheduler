<template>
  <div class="schedule-page">
    <div class="schedule-toolbar">
      <h2 class="page-title"><span class="title-icon">📅</span>教室课表</h2>
    ...
    <div v-if="!selectedRoomId" class="empty-tip"><el-empty description="请选择教室查看课表" /></div>
    ...
    <div v-else>
      <div class="info-bar room-info" v-if="selectedRoom">
        <span class="info-item"><span class="info-label">教室：</span><span class="info-value">{{ selectedRoom.name }}</span></span>
        <span class="info-sep">|</span>
        <span class="info-item"><span class="info-label">类型：</span><span class="info-value">{{ selectedRoom.room_type }}</span></span>
        <span class="info-sep">|</span>
        <span class="info-item"><span class="info-label">容量：</span><span class="info-value">{{ selectedRoom.capacity }}人</span></span>
        <span class="info-sep">|</span>
      ...
      <div class="schedule-wrapper" ref="scheduleRef">
        <table class="weekly-schedule">
          <thead>
            <tr><th class="period-col">节次</th><th v-for="(day, idx) in weekDays" :key="idx" class="day-col"><div class="day-header"><span class="day-name">{{ day.name }}</span><span class="day-date">{{ day.date }}</span></div></th></tr>
          </thead>
          <tbody>
              ...
              <tr v-for="(period, pIdx) in section.periods" :key="pIdx" :class="{ 'section-break': pIdx === section.periods.length - 1 }">
                ...
                <td class="schedule-cell" v-for="day in weekDays" :key="day.index">
                  <div v-if="getCellContent(day.index, period)" class="course-card" :style="getCardStyle(getCellContent(day.index, period))">
                    ...
                    <div class="course-room"><span class="icon">👥</span><span>{{ getCellContent(day.index, period).class_name }}</span></div>
                  </div>
...
const cellMap = computed(() => { const map = {}; courseData.value.forEach(c => { const key = `${c.day}-${c.period}`; if (!map[key]) map[key] = c }); return map })
...
function handlePrint() { printSchedule('教室课表') }

async function loadRoomList() {
  try {
    const res = await roomApi.getList({ page: 1, per_page: 100 })
    roomList.value = res.rooms || []
    if (roomList.value.length > 0 && !selectedRoomId.value) { selectedRoomId.value = roomList.value[0].id; loadSchedule() }
  } catch (e) { console.error(e) }
}

async function loadSchedule() {
  if (!selectedRoomId.value) { courseData.value = []; return }
  loading.value = true
  try {
    const res = await scheduleApi.getWeekly({ room_id: selectedRoomId.value, week: currentWeek.value })
    courseData.value = res.courses || []
    if (!courseData.value.length) ElMessage.warning('该教室本周暂无排课数据')
  } catch (e) { console.error(e); ElMessage.error('加载课表失败') } finally { loading.value = false }
}

onMounted(() => { loadRoomList() })
</script>

<style scoped>.room-info { background: linear-gradient(135deg, #ee0979 0%, #ff6a00 100%); }</style>

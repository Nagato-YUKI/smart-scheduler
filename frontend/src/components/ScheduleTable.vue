<template>
  <div class="schedule-table">
      ...
      <table class="schedule-grid">
        ...
        <tbody>
          <tr v-for="week in visibleWeeks" :key="week" class="week-row">
            ...
            >
              <div 
                v-for="course in getCourses(week, day.value)" 
                :key="course.id"
                class="course-card"
                :class="{
                  'is-evening': course.period === '晚上',
                  'is-afternoon': course.period === '下午',
                  'is-morning': course.period === '上午'
                }"
              >
    ...
    <div class="pagination-controls" v-if="totalWeeks > 5">
      <el-button-group>
        <el-button @click="changeRange(-1)" :disabled="startWeek <= 1">
          上一组
        </el-button>
        <el-button>
          第 {{ startWeek }}-{{ endWeek }} 周
        </el-button>
        <el-button @click="changeRange(1)" :disabled="endWeek >= totalWeeks">
          下一组
        </el-button>
      </el-button-group>
      <div class="quick-jump">
        ...
        <el-select v-model="currentStartWeek" @change="onWeekChange" size="small">
          <el-option 
            v-for="start in weekRanges" 
            :key="start" 
            :label="`第 ${start}-${Math.min(start + weeksPerView - 1, totalWeeks)} 周`"
            :value="start"
          />
        </el-select>
      </div>
...
const endWeek = computed(() => {
  return Math.min(startWeek.value + props.weeksPerView - 1, props.totalWeeks)
})

const visibleWeeks = computed(() => {
  const weeks = []
  for (let w = startWeek.value; w <= endWeek.value; w++) {
    weeks.push(w)
  }
  return weeks
})
...
const weekRanges = computed(() => {
  const ranges = []
  for (let i = 1; i <= props.totalWeeks; i += props.weeksPerView) {
    ranges.push(i)
  }
  return ranges
})
...
function getCourses(week, dayOfWeek) {
  return props.scheduleData.filter(course => {
    const weekMatch = course.week === week || 
      (course.startWeek && course.endWeek && week >= course.startWeek && week <= course.endWeek)
...
function changeRange(direction) {
  ...
  if (newStart >= 1 && newStart <= props.totalWeeks) {
    startWeek.value = newStart
    currentStartWeek.value = newStart
  }
}

function onWeekChange(value) {
  startWeek.value = value
}
</script>
...
.course-card.is-evening {
  border-left-color: #e6a23c;
  background-color: #fdf6ec;
}

.course-card.is-afternoon {
  border-left-color: #67c23a;
  background-color: #f0f9ff;
}

.course-card.is-morning {
...
.is-evening .period-badge {
  background-color: #e6a23c;
}

.is-afternoon .period-badge {
  background-color: #67c23a;
}

.is-morning .period-badge {
  background-color: #409EFF;
}
